...
class DCDetector(IDCDetector):
  ...
  def find_violations(self, df: DataFrame, dc: IDC) -> AdjacencyList:    
    ...
    return list(filter(lambda t: bool(t[1]), adjacency_list))  # type: ignore
          
  def __filter_by_scalar_predicates(self, df: DataFrame, scalar_predicates: List[Predicate]) -> DataFrame:
  ...
  def __filter_by_diff_target_predicates(self, targets: DataFrame, df: DataFrame, rel_predicates: List[Predicate]) -> DataFrame:
